...
export default function EstoquePage() {
  ...
  const distribuidos = cases.filter((c) => c.status === 'Distribuído')
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-6 py-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-brand-slate font-light mt-1">Produto: Cestas de Serviços</p>
      </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {cases.map((processo) => {
                ...
                return (
                  <tr
                    key={processo.id}
                    className={`hover:bg-gray-50 transition-colors ${isSelected ? 'bg-brand-offwhite' : ''}`}
                  >
                    ...
                    <td className="px-4 py-3">
                      {(processo.status === 'Em tratativa' || processo.status === 'Distribuído') &&
                       processo.analistaResponsavel === 'Ana Costa' ? (
                        <Link
